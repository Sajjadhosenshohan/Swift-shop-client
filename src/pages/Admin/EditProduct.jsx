...
const EditProduct = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const info = { _id, productName, brandName, category, productImage, description, price, sellingPrice }

    console.log(info)

    const response = await fetch("http://localhost:8000/api/update-product", {
      method: "PUT",
      credentials: "include",
      headers: {
      ...
      <div className="bg-white p-5 rounded-sm w-full max-w-[80%] h-full max-h-[80%]  text-slate-700">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto h-full pt-4 pb-10">
          <div className="flex flex-col gap-3">
            ...
            <div>
              <button type="submit" className="btn btn-form">Update Product</button>
            </div>
